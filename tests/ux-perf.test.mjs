...
const srcFiles = ["src/lib/supabaseClient.ts", "src/data/supabaseRepository.ts", "src/data/repositorySource.ts"]
  .map((f) => stripComments(read(f)));
check("ولا ملف يستورد المظلّة", srcFiles.every((f) => !/@supabase\/supabase-js/.test(f)));
check("ولا استدعاء auth باقٍ", srcFiles.every((f) => !/\.auth\./.test(f)));
check("ولا realtime ولا storage ولا rpc",
  srcFiles.every((f) => !/\.channel\(|realtime|storage\.|functions\.invoke|\.rpc\(/.test(f)));
